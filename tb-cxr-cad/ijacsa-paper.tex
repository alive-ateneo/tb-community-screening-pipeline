...
DraxNet is a custom model ($17.0$M parameters) based on a ResNet-18 backbone. The main modification is that the fourth layer consists of two custom blocks called \emph{DraxResidualBlocks}. All other components are unchanged from ResNet-18.
...
The block is inspired by the SymAttention mechanism of SymFormer~\cite{liu2023tbx11k_pami}, which observes that bilateral asymmetry in the chest radiograph is a key indicator of TB. The design goal is to test whether a simple, parameter-efficient symmetry-aware gate can capture the same bilateral-asymmetry cue as heavier spatial-transformer approaches. Hence, FlipR has roughly one-quarter the parameters of ResNet-18.

\subsection{Model Training}

All models are trained from scratch with a random seed of $42$, batch size of $16$, $512\times512$ input resolution, and $100$ epochs. Optimization uses the Adam optimizer with a fixed learning rate of $10^{-3}$, no learning-rate schedule, and an unweighted cross-entropy loss. No hyperparameter tuning was done.
