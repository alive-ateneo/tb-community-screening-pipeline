...
\begin{tabular}{@{}p{3.0cm} *{9}{c}@{}}
\toprule
\textbf{Feature} & \textbf{qXR}~\cite{qxr_docs} & \textbf{CAD4TB+}~\cite{cad4tb_product} & \textbf{Lunit}~\cite{lunit_product} & \textbf{DrAid}~\cite{draid_product} & \textbf{Genki}~\cite{genki_product} & \textbf{InferRead}~\cite{inferread_product} & \textbf{Annalise}~\cite{annalise_product} & \textbf{Siemens}~\cite{siemens_product} & \textbf{Proposed} \\
\midrule
...
Heatmap/overlay of localization findings  & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
Clinically grounded reasoning of findings in natural language &            &              &              & \textit{n/a} & \textit{n/a} &              &              &              & $\checkmark$ \\
\bottomrule
\end{tabular}
...
As a secondary contribution, we develop and benchmark two custom tuberculosis models, DraxNet and FlipR, for image-level classification and bounding-box localization, evaluated against SOTA baselines.
...
The TB classification module classifies each CXR as either healthy, sick non-TB, or TB as seen in Figure~\ref{fig:cxr_scans}.

\subsection{Dataset}

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{../figures/cxr-scans.png}
    \caption{Representative chest X-rays and labels from the TBX11K dataset.}
    \label{fig:cxr_scans}
\end{figure}
...
Active TB and latent TB are referred to in TBX11K as \textit{ActiveTuberculosis} and \textit{ObsoletePulmonaryTuberculosis} respectively. Active TB denotes ongoing infection that is transmissible and clinically urgent. Latent TB denotes prior, healed or inactive infection that is not contagious and does not require immediate treatment, although may be at risk of reactivation. As seen in Figure~\ref{fig:cxr_scans}, active TB is more immediately visible, and latent TB is more subtle.

In contrast to classification, which uses image-level labels, localization labels are at the bounding-box level. A CXR carrying an image-level TB label may contain only active-TB lesions, only latent-TB lesions, or lesions of both types on the same CXR, as depicted in Figure~\ref{fig:cxr_scans}.
...
\begin{figure*}[!t]
\centering
\subfloat{\includegraphics[width=0.42\textwidth]{../figures/pr_yolo26.pdf}}
\hfil
\subfloat{\includegraphics[width=0.42\textwidth]{../figures/f1_yolo26.pdf}}
\\
\subfloat{\includegraphics[width=0.42\textwidth]{../figures/pr_draxnet.pdf}}
\hfil
\subfloat{\includegraphics[width=0.42\textwidth]{../figures/f1_draxnet.pdf}}
